...
const addLikeState = async (comments, userId) =>{
  return Promise.all(
    comments.map(async (comment) => {
      const liked = await interactionRepository.isCommentLikedByUser(userId, comment._id);
      return {
        ...comment,
...
export const interactionService = {
  ...
  addComment: async (userId, postId, payload) => {
    ...
    if (parentCommentId) {
      ...
      if (parentComment.parentCommentId) {
        throw new AppError('NESTED_REPLIES_NOT_SUPPORTED', 'Replies can only be added to top-level comments.', 400);
      }
    }
  ...
  getComments: async (userId, postId, query) => {
    ...
    const comments = await interactionRepository.findCommentsByPost(postId, query.cursor, limit,userId);
    const commentsWithMeta = await addLikeState(comments, userId);
    return postService.toPaginationResult(commentsWithMeta, limit, getCreatedAtCursorFromDoc);
  },

  getReplies: async (commentId, query, userId) => {
    await ensureComment(commentId);
    const limit = getLimit(query.limit);
    const replies = await interactionRepository.findRepliesByComment(commentId, query.cursor, limit);
    const repliesWithMeta = await addLikeState(replies, userId);
    return postService.toPaginationResult(repliesWithMeta, limit, getCreatedAtCursorFromDoc);
  },
  ...
  likeComment: async (userId, commentId) => {
    ...
    try {
      ...
      if (isDuplicateKey(error)) {
        throw new AppError('COMMENT_LIKE_ALREADY_EXISTS', 'Comment is already liked.', 409);
      }
      throw error;
    }

    const updatedComment = await interactionRepository.incrementCommentLikeCount(commentId, 1);
  ...
  unlikeComment: async (userId, commentId) => {
    await ensureComment(commentId);
    const deleted = await interactionRepository.deleteCommentLike(userId, commentId);
    if (!deleted) {
      throw new AppError('COMMENT_LIKE_NOT_FOUND', 'Comment is not liked by this user.', 404);
    }
  ...
  updateComment: async (userId, commentId, content) => {
    const comment = await interactionRepository.updateComment(commentId, userId, content);
    if (!comment) {
      throw new AppError(
  ...
  deleteComment: async (userId, commentId) => {
    ...
    if (comment.parentCommentId) {
      await interactionRepository.incrementReplyCount(comment.parentCommentId, -1);
    }
